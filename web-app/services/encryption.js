 ...
 *
 * eg
 * Input, paramsToShare: ["departmentName", "prescription"].
 * ordering: ["doctorName", "disease", "departmentName", "prescription"]
 * Output: [2,3]
 *
 */
 ...
 * @param {String} patientEmail - Certiificate holder email. Used to invoke chaincode.
 * @returns {Promise<Buffer[]>} proofObject
 */
async function generateCertificateProof(paramsToShare, certUUID, patientEmail) {
    let certSchema = await chaincode.invokeChaincode("queryCertificateSchema",
        ["v1"], true, patientEmail);
