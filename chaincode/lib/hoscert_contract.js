...
const DoctorProfile = require('./doctor_profile');
const Schema = require('./schema');


class HoscertContract extends Contract {
    ...
    async initLedger(ctx) {
        console.log("-------------------------initialLedger---------------------------------------")
      
        let schemaCertificate = new Schema("doctor degree", "v1", ["doctorName", "disease", "departmentName", "prescription"] );
     ...
     */
    async issueCertificate(ctx, certHash, doctorSignature, patientSignature, dateOfIssuing, certUUID, doctorPK, patientPK) {
        console.log("============= START : Issue Certificate ===========");
        //todo: Validate data.

        const certificate = new Certificate(certHash, doctorSignature, patientSignature, dateOfIssuing, certUUID, doctorPK, patientPK);
    ...
    /**
    * Register a doctor. Must be done when a doctor enrolls into the platform.
    * @param {Context} ctx The transaction context
    * @param {String} name 
    ...
    */
    async registerDoctor(ctx, name, publicKey, location, description) {
        console.log("============= START : Register Doctor ===========");
        //todo Add validation.
        const doctor = new DoctorProfile(name, publicKey, location, description);
        await ctx.stub.putState("DOC" + name, Buffer.from(JSON.stringify(doctor)));

        console.log("============= END : Register Doctor ===========");
        return doctor;
    }

    /**
     * Get public profile of a enrolled doctor based on it's name
     * @param {Context} ctx The transaction context
     * @param {String} name 
     * @returns {JSON} doctor Profile
     */
    async queryDoctorProfileByName(ctx, name) {
        const profileAsBytes = await ctx.stub.getState("DOC" + name);

        if (!profileAsBytes || profileAsBytes.length === 0) {
            throw new Error(`Doctor ${name} does not exist`);
        }

        console.log(`Doctor ${name} Query Successful. Profile: `);
    ...
    /**
     * Returns all the certificates received by a specific patient
     * @param {Context} ctx The transaction context
     * @param {*} patientPK Public Key of patients account in platform
     * @returns {[Certificate]} 
     */
    async getAllCertificateByPatient(ctx, patientPK) {
        let queryString = {
            selector: {
                patientPK: patientPK,
        ...
        for (let i = 0; i < queryResults.length; i++) {
            try {
                ...
                console.log("Failed to instantiate Certificate object from JSON in getAllCertificateByPatient\n" + err);
    ...
    /**
     * Returns al the certificates issued by a specific doctor
     * @param {Context} ctx The transaction context
     * @param {*} doctorPK Public Key of doctor that issued  the certificate
     * @returns {[Certificate]} 
     */
    async getAllCertificateByDoctor(ctx, doctorPK) {
        let queryString = {
            selector: {
                doctorPK: doctorPK,
                dataType: "certificate"
        ...
        for (let i = 0; i < queryResults.length; i++) {
            try {
                ...
                console.log("Failed to instantiate Certificate object from JSON in getAllCertificateByDoctor\n" + err);
                console.log("DATA TYPE:  " + typeof queryResults[i])
                certArray.push(queryResults[i])
            }
        }
...
module.exports = HoscertContract;
